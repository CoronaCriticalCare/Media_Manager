...
class PhotoToolsApp(TkinterDnD.Tk if DND_AVAILABLE else tk.Tk):
    ...
    def _create_widgets(self):
        ...
        for tab_name, tab in self.tabs.items():
            ...
            if DND_AVAILABLE and tab_name in [
                "Media Discovery",
                "Scanned Albums",
                "Clean Upload",
            ]:
            ...
            if PIL_AVAILABLE and os.path.exists(logo_path):
                try:
                    img = Image.open(logo_path).resize(
                        (600, 300), Image.LANCZOS
                    )
                    photo = ImageTk.PhotoImage(img)
                    self.logo_photos[tab_name] = photo
                    tk.Label(logo_frame, image=photo, bg="lightgray").pack(
                        pady=10
                    )
                except Exception:
                    tk.Label(
                ...
                tk.Label(
                    logo_frame, text="[Logo not loaded]", bg="lightgray"
                ).pack()
        ...
        self.progress_label = tk.Label(
            self.progress_frame, text="0%", bg="lightgray"
        )
    ...
    def update_controls(self, tab_name):
        ...
        if tab_name == "Clean Upload":
            tk.Label(
                self.control_panel, text="No JSON loaded.", bg="white"
            ).pack(pady=5)
            tk.Button(
                self.control_panel,
                text="Load JSON",
            ...
            tk.Button(
                self.control_panel,
                text="Load Scanned",
                command=self.load_scanned,
            ).pack(pady=5)

            tk.Button(
                self.control_panel,
                text="Move Albums",
                command=self.move_albums,
            ).pack(pady=5)
    ...
    def face_match_mode(self):
        ...
        dest = filedialog.askdirectory(
            title="Select destination for matched."
        )
        if not dest:
    ...
    def _face_match_thread(
        self, targets, source_folder, matched_folder, threshold=0.6
    ):
        try:
            encs = recognition.build_target_encodings(
                targets, model="hog", log=self._log_console
            )

            if not encs:
                self._log_console(
                    "[FaceMatch] No valid target faces found."
                )
                return
    ...
    def _open_select_folder(self, tab_name):
        folder = filedialog.askdirectory(
            title="Select folder for: " + tab_name
        )
        if folder:
    ...
    def run_upload(self):
        ...
        if not src:
            self._log_console(
                "[Clean Upload] No folder dropped."
            )
            return
        ...
        if not dest:
            self._log_console(
                "[Clean Upload] No destination selected."
            )
            return

        self._log_console(
            f"[Clean Upload] Copy from {src} -> {dest}"
        )
    ...
    def _run_upload_thread(self, src, dest):
        try:
            clean_upload.batch_clean_upload(
                [src], dest, log=self._log_console
            )
    ...
    def scan_media(self):
        ...
        if not folder:
            self._log_console(
                "No folder dropped for Media Discovery."
            )
            return
        ...
        threading.Thread(
            target=self._scan_media_thread, args=(folder,), daemon=True
        ).start()
    ...
    def _collect_organize_inputs(self):
        ...
        if not base_path:
            self._log_console(
                "[Media Organizer] No folder selected."
            )
            return

        folder_name = askstring(
            "Organized Album",
            "Enter name of organized folder: ",
        )
        if not folder_name:
    ...
    def _organize_media_thread(
        self, json_path, base_path, folder_name
    ):
        try:
            media_dict = cross_pic_organizer.load_media_json(
                json_path, log=self._log_console
            )
            if not media_dict:
                self._log_console(
                    "[Media Organizer] Empty JSON."
                )
                return
    ...
    def load_scanned(self):
        ...
        if not folder:
            self._log_console(
                "[Scanned Albums] No folder dropped."
            )
            return

        album_name = askstring(
            "Default Album", "Enter default album name:"
        )
        if not album_name:
            self._log_console(
                "[Scanned Albums] Album name required."
            )
            return

        tags_input = askstring(
        ...
        if not date_start or not date_end:
            self._log_console(
                "[Scanned Albums] Missing dates."
            )
            return

        self._log_console(
            f"[Scanned Albums] Date range {date_start} -> {date_end}"
        )
